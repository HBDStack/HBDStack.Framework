...
public static class CollectionExtenstion
{
        ...
        where T : class where TKey : struct
    {
        ...
        return (from i in @this
            from y in @this
            where y != null && i != y && keySelector(i).Equals(keySelector(y))
            select i).Any();
    }
        ...
        where T : class
    {
        ...
        return (from i in @this
            from y in @this
            where i != null && y != null && i != y && keySelector(i).Equals(keySelector(y), StringComparison.OrdinalIgnoreCase)
            select i).Any();
    }
    ...
    public static void RemoveRange<T>(this ICollection<T> @this, IEnumerable<T> collection)
    {
        ...
        if (@this.IsReadOnly)
        {
            const string Message = "The read-only collection can't remove items.";
